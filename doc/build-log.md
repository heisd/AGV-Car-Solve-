...
## 13. 默认 2 车 + 系统异常总览面板 + 让行策略修复

### 默认改为 2 车（重要决策）
- `warehouse_fleet_nav2.launch.py` 的 `num_agvs` 默认 **3→2**。
- 原因：本机 WSL2(8G/16核) 同时拉 **3** 套完整 Nav2 时，第 3 套(agv3) 的组合节点偶发 `load_node` 超时
  （amcl / bt_navigator 加载失败）→ 导航栈起不全 → `导航未就绪`、小车不动。15s 大错峰能改善但不彻底。
  **2 车在本机完全稳定可靠**（nav_ready 秒到 2/2、导航/回充/让行均正常）。需要 3 车可显式 `num_agvs:=3`，
  并用 `nav_ready` 红标观察第 3 套是否起全。

### 系统异常总览面板（前端统一异常显示）
- `fleet_manager` 在 `/fleet/state` 新增 `anomalies` 列表（后端聚合，权威）：
  - **碰撞**(error)：两车中心距 < 0.55m；**离线**(error)：无位姿/未上报；
  - **导航未就绪**(warn)：Nav2 未激活；**低电量**(warn)：<20% 且未充电；
  - **导航卡死**(warn)：朝目标长时间无进展（`_resend` 看门狗触发，新增 `AGVState.stuck`）；
  - **让行中**(info)：路径冲突优先级低、正在让行。
- 前端新增 **"系统异常"面板**（侧栏置顶）：按 error/warn/info 配色列出当前所有异常，标题处汇总
  "N 严重 / M 警告"，无异常显示绿色"正常"。实测稳定后能正确显示
  `[error]碰撞 / [warn]低电量 / [warn]导航卡死 / [info]让行中` 并在解除后清空。

### 让行策略修复（碰撞逻辑核心 bug）
22. **"原地停车"让行会堵路相撞**（实测复现）：低优先级车 `send_goal_xy(当前位姿)` 原地停，
    但它正停在高优先级车的必经单行道(x=±5.5)上 → 高优先级车（如去充电的 agv1）直接撞上停着的车
    (实测间距降到 0.33m)，且高优先级车被堵到 `导航卡死`、连充电都到不了。让行反而制造障碍。
    → **修复**：让行动作改为 **横向让到走廊内侧** `yield_pullaside_target()`——东/西走廊(|x|大)的车朝
    中心(x=0)横移 1.1m 让出 x=±5.5 单行道（南/北走廊同理沿 y），高优先级车即可沿原车道通过；
    冲突解除后 `reissue_goal()` 恢复原任务目标。
